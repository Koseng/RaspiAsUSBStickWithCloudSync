...
echo "-----------------------------------------------------------------"
sudo grep -qxF '#otg_mode=1' /boot/config.txt || sudo sed -i '/^otg_mode=1/s/^/#/' /boot/config.txt
sudo sh -c "grep -qxF 'dtoverlay=dwc2' /boot/config.txt || echo 'dtoverlay=dwc2' >> /boot/config.txt"
